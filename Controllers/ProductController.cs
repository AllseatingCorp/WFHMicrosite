...
namespace WFHMicrosite.Controllers
{
    public class ProductController : Controller
    {
        ...
        public IActionResult Index(int id, int siteid)
        {
            ...
            json = SendWebApiMessage(apiUrl + "sites/" + siteid, "GET", "");
            data.Site = JsonConvert.DeserializeObject<SiteModel>(json);
            json = SendWebApiMessage(apiUrl + "products/" + siteid, "GET", "");
            data.Products = JsonConvert.DeserializeObject<List<ProductModel>>(json);
            data.Product = data.Products.Where(x => x.ProductId == data.User.ProductId).FirstOrDefault();
            ...
            {
                item.Default = item.ProductId == data.User.ProductId ? true : false;
            }
            ...
            ViewBag.Logo = data.Site.LogoImage;
            ViewBag.Logo2 = data.Site.LogoImage2;
            return View(data);
        }
        ...
        {
            if (!string.IsNullOrWhiteSpace(data.User.PhoneNumber))
            {
            ...
            json = SendWebApiMessage(apiUrl + "sites/" + data.Site.SiteId, "GET", "");
            data.Site = JsonConvert.DeserializeObject<SiteModel>(json);
            json = SendWebApiMessage(apiUrl + "products/" + data.Site.SiteId, "GET", "");
            data.Products = JsonConvert.DeserializeObject<List<ProductModel>>(json);
            data.Product = data.Products.Where(x => x.ProductId == data.User.ProductId).FirstOrDefault();

            if (data.User.ProductId != data.ProductId)
            {
                data.User.ProductId = data.ProductId;
                update = false;
                redirect = true;
            }
            ...
            if (redirect)
            {
                return RedirectToAction("Index", new { id = data.User.UserId, siteid = data.Site.SiteId });
            }
        ...
        public IActionResult Complete(int id)
        {
            ...
            json = SendWebApiMessage(apiUrl + "sites/" + data.Product.SiteId, "GET", "");
            data.Site = JsonConvert.DeserializeObject<SiteModel>(json);
            json = SendWebApiMessage(apiUrl + "productoptions/" + data.User.ProductId + "/Fabric", "GET", "");
            data.FabricOptions = JsonConvert.DeserializeObject<List<ProductOptionModel>>(json);
            json = SendWebApiMessage(apiUrl + "productoptions/" + data.User.ProductId + "/Mesh", "GET", "");
